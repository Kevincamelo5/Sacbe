...
func _generar_problema() -> bool:
	...
	# Generamos un divisor y un resultado (cociente) entre 1 y 12
	# Esto asegura que la división sea exacta y simple
	var divisor := randi_range(1, 12)
	var resultado := randi_range(1, 12)
	
	# El número principal (dividendo) es la multiplicación de ambos
	var dividendo := divisor * resultado
	
	# Agregamos los operandos en el orden correcto (Ej: 24 ÷ 3)
	_operandos.push_back(dividendo)
	_operandos.push_back(divisor)
	
	# Guardamos el resultado esperado
	_respuesta_correcta = str(resultado)
...
func _es_respuesta_incorrecta() -> void:
	...
	# Crear fondo semitransparente verde
	var color_rect := ColorRect.new()
	color_rect.color = Color(0.588, 0.11, 0.344, 0.0)  # Verde más suave # El comentario dice verde pero el color es rojizo
	color_rect.set_anchors_preset(Control.PRESET_FULL_RECT)
	canvas.add_child(color_rect)
